...
import           Pos.Core.Slotting (MonadSlotsData, SlottingData,
                     slottingVar, SlotId (..), localSlotIndexMinBound)
        ...
        lastAdopted <- do
            ...
            case initialEra of
                OBFT _ -> do
                    ...
                    when (epochIndex /= minBound && slotIndex == localSlotIndexMinBound) $ do
                        logDebug $ "usVerifyBlocks OBFT: We're on epoch boundary. Running processGenesisBlock"
                        processGenesisBlock genesisConfig epochIndex
                    getAdoptedBV
